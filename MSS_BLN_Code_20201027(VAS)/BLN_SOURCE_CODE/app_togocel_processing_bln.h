...
char el_Destination_pre[3];  
char el_BNumber_pre[3]; 
...
char VAS_Flag[30];   
char VAS_Type[30];   
char EL_VAS_IN_TRUNKID[30];
char EL_VAS_OUT_TRUNKID[30];
char EL_VAS_Special_Dial_Case[30];
char EL_VERT_Dial_Case [30];
char VAS_Subscriber_Type[30];
char SP_VAS_Flag[30];  
char USSD_PARAM[50];
...
int  get_VAS_VERT_Number_Details(int lookupTableID, const char* MSISDN);
int get_VAS_Special_Number_Details(int lookupTableID, const char* MSISDN);
